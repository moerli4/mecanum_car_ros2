...
class HeadlightDriverNode(Node):
    def __init__(self):
        ...
        self.srv = self.create_service(SetHeadlights, "set_headlights", self.set_headlights_callback)
    ...
    def set_headlights_callback(self, request, response):
        ...
        self.get_logger().info(
            f"[Set Headlights] Incoming request {request.id, request.r, request.g, request.b}"
        )
        return response
